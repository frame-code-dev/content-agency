        ...
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div class="flex items-center space-x-3">
                <div class="px-4 py-2 bg-white border border-zinc-200 rounded-[10px] shadow-sm text-xs font-bold text-zinc-800 flex items-center space-x-2">
                    ...
                    <span>Engagement Rate: {{ $insights['engagement_rate'] ?? '6.48%' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <div class="bg-white border border-zinc-200 rounded-[10px] p-6 shadow-sm">
                ...
                <div class="mt-4 flex items-baseline justify-between">
                    <span class="text-3xl font-bold font-heading text-zinc-950 tracking-tight">{{ $insights['engagement_rate'] ?? '6.48%' }}</span>
            ...
            <div class="bg-white border border-zinc-200 rounded-[10px] p-6 shadow-sm">
                ...
                <div class="mt-4 flex items-baseline justify-between">
                    <span class="text-3xl font-bold font-heading text-zinc-950 tracking-tight">{{ number_format(array_sum(array_column($posts, 'like_count'))) }}</span>
            ...
            <div class="bg-white border border-zinc-200 rounded-[10px] p-6 shadow-sm">
                ...
                <div class="mt-4 flex items-baseline justify-between">
                    <span class="text-3xl font-bold font-heading text-zinc-950 tracking-tight">{{ number_format(array_sum(array_column($posts, 'comments_count'))) }}</span>
            ...
            <div class="bg-white border border-zinc-200 rounded-[10px] p-6 shadow-sm">
                ...
                <div class="mt-4 flex items-baseline justify-between">
                    <span class="text-3xl font-bold font-heading text-zinc-950 tracking-tight">{{ is_numeric($insights['reach'] ?? null) ? number_format($insights['reach']) : ($insights['reach'] ?? '1.1M') }}</span>
                            ...
                            <tr class="hover:bg-zinc-50 transition">
                                <td class="py-3.5 px-4 w-16">
                                    @if(isset($post['media_url']))
                                        <img src="{{ $post['media_url'] }}" class="w-10 h-10 rounded-[10px] object-cover ring-1 ring-zinc-200" alt="Post">
                                    @else
                                        <div class="w-10 h-10 rounded-[10px] bg-zinc-100 flex items-center justify-center font-bold text-[10px]">IG</div>
                                    @endif
                                </td>
                                <td class="py-3.5 px-4 max-w-sm truncate text-zinc-900 font-medium">{{ $post['caption'] ?? 'No caption' }}</td>
                                <td class="py-3.5 px-4 text-zinc-900 font-bold">❤️ {{ number_format($likes) }}</td>
                                <td class="py-3.5 px-4 text-zinc-900 font-bold">💬 {{ number_format($comments) }}</td>
