...
<?php $component->withAttributes([]); ?>
    <div class="p-8 max-w-[1600px] mx-auto space-y-8">
        ...
        <?php if($account && method_exists($account, 'isTokenExpired') && $account->isTokenExpired()): ?>
            <div class="bg-amber-500/10 border border-amber-500/30 p-4 rounded-2xl flex items-center justify-between text-amber-900 shadow-sm">
                ...
                <a href="<?php echo e(route('instagram.connect')); ?>" class="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl shadow transition">
                    🔄 Reconnect Instagram
                </a>
            </div>
        <?php endif; ?>
        ...
        <?php if(!$account): ?>
            ...
            <div class="max-w-3xl mx-auto my-12 bg-white border border-slate-200/80 rounded-3xl p-12 text-center shadow-lg">
                ...
                <div class="mt-8">
                    <a href="<?php echo e(route('instagram.connect')); ?>" class="inline-flex items-center space-x-3 px-8 py-4 bg-[#072215] hover:bg-[#051910] text-[#A3E635] text-sm font-bold tracking-wide rounded-2xl shadow-xl hover:scale-105 transition transform duration-200">
        ...
        <?php else: ?>
            ...
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="flex items-center space-x-2 bg-white border border-slate-200/80 px-3 py-1.5 rounded-2xl text-xs shadow-sm">
                        <span class="w-2 h-2 rounded-full <?php echo e(method_exists($account, 'isTokenExpired') && $account->isTokenExpired() ? 'bg-amber-500 animate-ping' : 'bg-emerald-500 animate-pulse'); ?>"></span>
                        <span class="font-semibold text-slate-700 font-mono"><?php echo e('@' . ($account->username ?? 'account')); ?></span>
                        
                        <!-- Reconnect Button -->
                        <a href="<?php echo e(route('instagram.connect')); ?>" title="Reconnect Instagram to refresh token" class="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-bold rounded-xl text-[11px] border border-emerald-200 transition">
                            🔄 Reconnect
                        </a>

                        <!-- Disconnect Button Form -->
                        <form action="<?php echo e(route('instagram.disconnect')); ?>" method="POST" class="inline" onsubmit="return confirm('Disconnect this Instagram account?')">
                            <?php echo csrf_field(); ?>
                    ...
                    <div class="flex items-center space-x-4 bg-white border border-slate-200/80 p-3 px-5 rounded-2xl shadow-sm">
                        <img class="w-11 h-11 rounded-full object-cover ring-2 ring-[#A3E635]" src="<?php echo e($account->profile_picture_url ?? 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80'); ?>" alt="Avatar">
                        <div class="pr-4 border-r border-slate-200">
                            <div class="text-sm font-bold text-slate-900 leading-tight"><?php echo e($account->name ?? $account->username ?? $user->name ?? 'Cineart Client'); ?></div>
                            <div class="text-[11px] text-emerald-600 font-semibold font-mono"><?php echo e('@' . ($account->username ?? 'account')); ?></div>
                        </div>
                        <div class="flex items-center space-x-5 text-center pl-2">
                            <div>
                                <div class="text-sm font-bold text-slate-900"><?php echo e(number_format($insights['followers_count'] ?? 0)); ?></div>
                                <div class="text-[10px] text-slate-400 font-medium">Followers</div>
                            </div>
                            <div>
                                <div class="text-sm font-bold text-slate-900"><?php echo e(number_format($insights['follows_count'] ?? 0)); ?></div>
                                <div class="text-[10px] text-slate-400 font-medium">Following</div>
                            </div>
                            <div>
                                <div class="text-sm font-bold text-slate-900"><?php echo e(number_format($insights['media_count'] ?? count($posts))); ?></div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <div class="bg-white border border-slate-200/70 rounded-3xl p-6 shadow-sm hover:shadow-md transition duration-200">
                    ...
                    <div class="mt-4 flex items-baseline justify-between">
                        <span class="text-3xl font-bold font-heading text-slate-900 tracking-tight"><?php echo e(number_format($insights['followers_count'] ?? 0)); ?></span>
                ...
                <div class="bg-white border border-slate-200/70 rounded-3xl p-6 shadow-sm hover:shadow-md transition duration-200">
                    ...
                    <div class="mt-4 flex items-baseline justify-between">
                        <span class="text-3xl font-bold font-heading text-slate-900 tracking-tight"><?php echo e($insights['engagement_rate'] ?? '6.48%'); ?></span>
                        <span class="inline-flex items-center text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md">
                            ↑
                        </span>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="lg:col-span-3 bg-white border border-slate-200/70 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <?php $__currentLoopData = $days; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dIdx => $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="flex items-center space-x-1.5">
                                <span class="w-5 text-[10px] font-bold text-slate-400 text-right shrink-0"><?php echo e($day); ?></span>
                                <div class="grid grid-cols-9 gap-1 flex-1">
                                    <?php $__currentLoopData = $hours; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hIdx => $hour): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php
                                        ...
                                        ?>
                                        <div class="h-5 rounded-md <?php echo e($bgClass); ?> transition hover:scale-110" title="Activity score: <?php echo e($val); ?>"></div>
            ...
            <div class="bg-white border border-slate-200/70 rounded-3xl p-6 shadow-sm">
                <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
                    ...
                    <span class="text-xs font-semibold text-slate-400 font-mono"><?php echo e(count($posts)); ?> Posts Available</span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php $__empty_1 = true; $__currentLoopData = $posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="bg-slate-50/70 border border-slate-200/80 rounded-2xl overflow-hidden flex flex-col justify-between hover:shadow-md transition duration-200">
                            <div>
                                <?php if(isset($post['media_url']) && (str_contains($post['media_type'] ?? '', 'IMAGE') || str_contains($post['media_type'] ?? '', 'VIDEO'))): ?>
                                    <div class="aspect-square w-full bg-slate-200 overflow-hidden relative">
                                        <?php if(str_contains($post['media_type'] ?? '', 'VIDEO')): ?>
                                            <video src="<?php echo e($post['media_url']); ?>" class="w-full h-full object-cover" muted loop></video>
                                        ...
                                        <?php else: ?>
                                            <img src="<?php echo e($post['media_url']); ?>" alt="Post Media" class="w-full h-full object-cover">
                                        <?php endif; ?>
                                    </div>
                                <?php else: ?>
                                    <div class="aspect-square w-full bg-slate-100 flex items-center justify-center p-6 text-center">
                                        <span class="text-xs font-bold uppercase tracking-widest text-slate-400"><?php echo e($post['media_type'] ?? 'CAROUSEL'); ?></span>
                                    </div>
                                <?php endif; ?>

                                <div class="p-5">
                                    <p class="text-xs text-slate-700 line-clamp-3 leading-relaxed">
                                        <?php echo e($post['caption'] ?? 'No caption provided.'); ?>
                            ...
                            <div class="p-5 pt-0 space-y-3">
                                <div class="flex items-center justify-between text-xs text-slate-600 font-semibold border-t border-slate-200/60 pt-3">
                                    <span>❤️ <?php echo e(number_format($post['like_count'] ?? 0)); ?> Likes</span>
                                    <span>💬 <?php echo e(number_format($post['comments_count'] ?? 0)); ?> Comments</span>
                                </div>

                                <form action="<?php echo e(route('analysis.process')); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                                    ...
                                    <input type="hidden" name="likes" value="<?php echo e($post['like_count'] ?? 0); ?>">
                                    <input type="hidden" name="comments" value="<?php echo e($post['comments_count'] ?? 0); ?>">
                ...
                document.addEventListener("DOMContentLoaded", function () {
                    ...
                    if (ctxGrowth) {
                        ...
                        new Chart(ctxGrowth, {
                            type: 'line',
                            data: {
                                labels: <?php echo json_encode($insights['trend_labels'] ?? ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul']); ?>,
                                datasets: [{
                                    label: 'Follower Growth',
                                    data: <?php echo json_encode($insights['trend_data'] ?? [12000, 15000, 14000, 22000, 26000, 21000, 24000]); ?>,
                                    borderColor: '#84CC16',
                                    borderWidth: 3,
                    ...
                    if (ctxVisitors) {
                        new Chart(ctxVisitors, {
                            type: 'bar',
                            data: {
                                labels: <?php echo json_encode($insights['visitors_labels'] ?? ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']); ?>,
                                datasets: [{
                                    data: <?php echo json_encode($insights['visitors_data'] ?? [4200, 4800, 4100, 5000, 4600, 4900]); ?>,
